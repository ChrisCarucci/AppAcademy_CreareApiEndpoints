  ...
  req.on("end", () => { // request is finished assembly the entire request body
    ...
    if (req.method === 'GET' && req.url.startsWith('/dogs/')) {
      ...
      if (urlParts.length === 3) {
        ...
        // Your code here
        const dog = dogs.find(dog => dogId === dogId)
          res.statusCode = 200;
          res.setHeader("Content-Type", "application/json")
      ...
      if (urlParts.length === 3) {
        ...
        const dog = dogs.map((dog) => {
          if(dog.dogId === dogId) {
            ...
            return `Dog with ${dogId} does not exist, sorry!`
          }
        })
    ...
    if (req.method === 'DELETE' && req.url.startsWith('/dogs/')) {
      ...
      if (urlParts.length === 3) {
        const dogId = urlParts[2];
        // Your code here
        const message = dog.map((dog, index) => {
          if (dog.Id === dogId) {
            dogs.splice(index, 1);
            return "Successfully Deleted the dog, poor pup. :("
          } else {
            return `No dog with ${dogId} exists.`
          }
        })
        res.statusCode = 200;
        res.setHeader("Content-Type", "application/json")
